/**
 * Facebook Detection Plugin - v2.0
...
export function createFacebookPlugin(): DetectionPlugin {
  return {
    ...
    init(platformConfig: any): void {
      ...
      console.log('[Facebook Plugin v2] Initializing with layered detection pipeline');
      
      if (config.debug) {
        console.log('[Facebook Plugin v2] Config:', config);
      }
    ...
    teardown(): void {
      ...
      config = null;
      console.log('[Facebook Plugin v2] Teardown complete');
    },
  };
}
...
function detectFacebookAds() {
  if (!config) {
    console.warn('[Facebook Plugin v2] No config available');
    return;
  }
  ...
  try {
    ...
    console.log(`[Facebook Plugin v2] Detected ${candidates.length} new ads`);
    ...
    console.error('[Facebook Plugin v2] Detection error:', error);
  }
}
...
function sendAdCandidate(candidate: AdCandidate) {
  ...
  if (config?.debug) {
    console.log('[Facebook Plugin v2] Sending ad candidate:', payload);
  }
